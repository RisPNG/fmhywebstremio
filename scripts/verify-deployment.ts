...
async function verifyDeploymentContract(): Promise<void> {
  ...
  try {
    ...
    const configuredManifestResponse = await fetch(`http://127.0.0.1:${port}/disabled=bcine:bcine.ru/manifest.json`);
    const configuredManifest = await configuredManifestResponse.json() as { config?: { key: string; default?: string }[] };
    if (!configuredManifestResponse.ok || configuredManifest.config?.find(field => field.key === 'disableFmhySource_bcine:bcine.ru')?.default !== 'checked') throw new Error('Clean configuration path contract is invalid');
    const configure = await fetch(`http://127.0.0.1:${port}/configure`);
    const configureBody = await configure.text();
    if (!configure.ok || !configure.headers.get('content-type')?.includes('text/html') || !configureBody.includes('FMHY\'s Website Streamer') || !configureBody.includes('Provides video HTTP URLs from streaming websites listed by FMHY.') || !configureBody.includes('>67movies.nl</div>') || !configureBody.includes('>bcine.ru</div>') || !configureBody.includes('>cinego.co</div>') || configureBody.includes('cineby.at') || configureBody.includes('7movies.in') || /torbox|debrid|debris|MediaFlow|WebStreamrMBG/i.test(configureBody)) throw new Error('Configuration endpoint contract is invalid');
    ...
    if (!live.ok || liveBody.status !== 'ok' || liveBody.details?.['67movies.nl'] !== 'healthy' || liveBody.details?.['bcine.ru'] !== 'healthy' || liveBody.details?.['cinego.co'] !== 'healthy' || 'cineby.at' in (liveBody.details ?? {}) || '7movies.in' in (liveBody.details ?? {})) throw new Error('Runtime source registry was not loaded');
