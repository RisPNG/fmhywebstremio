...
async function verifyDeploymentContract(): Promise<void> {
  ...
  try {
    ...
    if (!manifestResponse.ok || manifest.id !== 'fmhy-webstream' || manifest.version !== '1.8.0' || manifest.name !== 'FMHY\'s Website Streamer' || !manifest.description?.startsWith('Provides video HTTP URLs from streaming websites listed by FMHY.') || !manifest.resources?.length || JSON.stringify(manifest.config?.map(field => field.key).sort()) !== JSON.stringify(sourceKeys) || !sourceKeys.includes('disableFmhySource_soapgo:soapgo.to')) throw new Error('Manifest contract is invalid');
    const configuredManifestResponse = await fetch(`http://127.0.0.1:${port}/disabled=soapgo:soapgo.to/manifest.json`);
    const configuredManifest = await configuredManifestResponse.json() as { config?: { key: string; default?: string }[] };
    if (!configuredManifestResponse.ok || configuredManifest.config?.find(field => field.key === 'disableFmhySource_soapgo:soapgo.to')?.default !== 'checked') throw new Error('Clean configuration path contract is invalid');
