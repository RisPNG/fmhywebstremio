...
async function verifyDeploymentContract(): Promise<void> {
  ...
  try {
    ...
    if (!live.ok || liveBody.status !== 'ok' || liveBody.details?.['67movies.nl'] !== 'healthy' || liveBody.details?.['cinebytv.com'] !== 'healthy' || liveBody.details?.['cinego.co'] !== 'healthy' || 'cineby.at' in (liveBody.details ?? {}) || 'bcine.ru' in (liveBody.details ?? {}) || '7movies.in' in (liveBody.details ?? {})) throw new Error('Runtime source registry was not loaded');
