...
async function verifyDeploymentContract(): Promise<void> {
  ...
  try {
    ...
    if (!manifestResponse.ok || manifest.id !== 'fmhy-webstream' || manifest.version !== '1.7.1' || manifest.name !== 'FMHY\'s Website Streamer' || !manifest.description?.startsWith('Provides video HTTP URLs from streaming websites listed by FMHY.') || !manifest.resources?.length || JSON.stringify(manifest.config?.map(field => field.key).sort()) !== JSON.stringify(sourceKeys) || !sourceKeys.includes('disableFmhySource_bingebang:bingebang.tv')) throw new Error('Manifest contract is invalid');
