...
describe('writeDeploymentSourceRegistry', () => {
  test('deterministically ships only runtime-eligible sources', async () => {
    ...
    registry.set({ id: 'healthy:healthy.test', canonicalDomain: 'healthy.test', aliases: [], fmhy: { section: 'Streaming', tags: ['Movies'], firstSeenAt: new Date('2026-01-01'), lastSeenAt: new Date('2026-01-02') }, family: { id: 'cinrift', confidence: 1, evidence: [], lastProbedAt: new Date('2026-01-02') }, status: 'supported' });
