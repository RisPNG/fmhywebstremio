...
The implemented reusable families include Cineby, CineGo, Cinetaro, 67Movies, P-Stream-compatible frontends, and Dooplay sites. Cineby uses exact TMDB-backed catalog pages and its Speedracelight playback architecture. CineGo and 67Movies share the VidsrcMe playback architecture while keeping their own exact catalog matching. P-Stream sites remain non-eligible when the provider architecture cannot find the known health-corpus media from the deployment network. Dooplay sites remain non-eligible when search discovery is incomplete or delegated players fail fresh validation. Browser-only anti-bot challenges, authenticated sites, client applications without a server-reproducible playback contract, and hosts that return expired media remain typed blocked, unsupported, failed, or inconclusive results rather than passes.
